...
def transform_raw_dataset(dataset_file_path: str=None) -> None:
    """
    ...
    #(1° critério) > remover dados que contém exercise|examples|doc
    df = pd.read_csv(dataset_file_path,sep=";")
    df = df[~df.FILE.str.contains("exercise|examples|doc")] # remove arquivos com exercises|examples

    metric_cols = ['LOC','COM','BLK','NOF','NOC','APF','AMC','NER','NEH','CYC','MAD','BUG']
    
    #(2° critério) > filtrar valores que são a maioria em zero, i.e, 50% das métricas são zero
    zero_threshold = len(metric_cols) // 2
    df['has_mostly_zeros'] = (df[metric_cols] == 0).sum(axis=1) > zero_threshold
    
    #(3° critério) > remoção de outliers usando métricas boxplot
    # Inicializa uma série booleana
    is_outlier_series = pd.Series(False, index=df.index)
    ...
    for col in metric_cols:
        ...
        # Identifica outliers na coluna atual
        # Para métricas de software, geralmente só nos preocupamos com outliers superiores (valores muito altos)
        col_outliers = (df[col] > upper_bound)
        # atualização acumulativa com OU lógico        
        is_outlier_series = is_outlier_series | col_outliers

    # Adiciona a coluna de flag de outlier ao DataFrame
    df['is_outlier'] = is_outlier_series
    df = df[df.is_outlier != True]
    df.to_csv(f'{transformed_file_path}.csv')
...
def start():
    ...
    transformed_dataset_path = transform_raw_dataset(dataset_file_path=raw_dataset_path)
